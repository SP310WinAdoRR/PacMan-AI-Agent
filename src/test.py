...
class PacManRetroEnv(gym.Env):
    ...
    def preprocess(self, obs):
        ...
        return obs.astype(np.float32)[None, :, :] / 255.0  # (1, 84, 84)

    def calculate_reward(self, ram):
        reward = 0
        curr_score = ram[0x4F] * 100  # NES memory: 0x4F = score unit
        delta = curr_score - self.prev_score

        # You can refine this if you decode more score details
        if delta == 10:
            reward = 10   # Pellet
        elif delta == 50:
            reward = 50   # Power Pellet
        elif delta >= 200:
            reward = 200  # Ghost eaten

        self.prev_score = curr_score
        return reward

    def risk_signal(self, ram):
        # Risk heuristic: Check if ghost is close by
        pacman_x = ram[0x44]
        pacman_y = ram[0x4C]
        ghosts_x = [ram[0x6E], ram[0x70], ram[0x72], ram[0x74]]
        ghosts_y = [ram[0x88], ram[0x8A], ram[0x8C], ram[0x8E]]

        for gx, gy in zip(ghosts_x, ghosts_y):
            dist = abs(pacman_x - gx) + abs(pacman_y - gy)
            if dist < 6:  # Arbitrary proximity threshold
                return 1  # Risk
        return 0
